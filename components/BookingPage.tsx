...
export function BookingPage() {
  const [serviceCode, setServiceCode] = useState(SERVICES[0].itemCode);
  ...
  const selectedService =
    SERVICES.find((service) => service.itemCode === serviceCode) || SERVICES[0];
  ...
  async function submitBooking(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage({ text: "預約已送出，請等候確認聯繫。", type: "success" });
    setCustomerName("");
    setCustomerPhone("");
    setNote("");
    setSelectedStart(null);
    ...
    <>
      <header className="site-header">
        <nav className="nav" aria-label="主選單">
          <a className="brand" href="#top" aria-label="襪子先生採耳首頁">
      ...
      <footer className="footer">襪子先生採耳｜線上預約</footer>
    </>
  );
}
