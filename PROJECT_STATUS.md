...
- 一般瀏覽器預約需填寶貝資料並會建立 `child_profiles`；LINE LIFF 進入可讀取既有寶貝資料或新增寶貝。
...
- 陪玩寶貝建檔需在 Supabase Production 執行 `supabase_child_profiles.sql`；此腳本包含 `child_profiles`、`booking_child_profiles` 與 `bookings.child_profile_id`，且 `child_profiles.line_user_id` 需允許為空，未執行前 web 預約建立寶貝資料與多寶貝關聯不會完整生效。
...
- 2026-05-14：修正一般 web 預約沒有 LIFF `line_user_id` 時不建立 `child_profiles` 的問題，commit `e5cab02 Create child profiles for web bookings` 已推送到 `main`。
- 2026-05-14：`supabase_child_profiles.sql` 更新為允許 `child_profiles.line_user_id` 為空；新版 `npm run build` 與 `npx tsc --noEmit --pretty false` 通過。
...
- 依 Manual Test Checklist 驗收陪玩頁：一般瀏覽器必填寶貝資料並寫入 `child_profiles`、LIFF 選既有寶貝、小團 3-4 位、多寶貝資料、三項服務計價、店家 LINE 通知內容。
